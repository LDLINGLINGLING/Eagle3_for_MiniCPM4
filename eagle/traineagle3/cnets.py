...
""" 
EAGLE模型实现 - 基于LLaMA的草稿模型（Draft Model）
EAGLE是一种推测解码（Speculative Decoding）的方法，通过训练一个轻量级的草稿模型来预测下一个token，
从而加速大模型的推理过程。
"""
...
class LlamaAttention(nn.Module):
    """
    多头注意力机制 - EAGLE版本
    与标准LlamaAttention的主要区别：
    1. 输入维度是hidden_size * 2 (连接了embedding和hidden states)
    2. 使用缓存的key/value状态
    3. 特殊的注意力权重计算方式
    """
    ...
    def __init__(self, config):
        ...
        # 注意：这里的输入维度是 hidden_size * 2，因为会连接embedding和hidden states
    ...
    def forward(
            ...
            cache_hidden: Optional[List[torch.Tensor]] = None,  # 缓存的key/value状态
...
class LlamaDecoderLayeremb(nn.Module):
    ...
    EAGLE的解码器层
    主要特点：
    1. 接受两个输入：input_emb（embedding）和hidden_states（隐藏状态）
    2. 将两者连接后送入注意力层
    3. 支持缓存机制
    """
    ...
    def forward(
            self,
            input_emb: torch.Tensor,        # Token embeddings
            hidden_states: torch.Tensor,    # 从目标模型提取的隐藏状态
            cache_hidden: [List[torch.Tensor]] = [],  # 缓存的key/value
            attention_mask: Optional[torch.Tensor] = None,
            position_ids: Optional[torch.LongTensor] = None,
            past_key_value: Optional[Tuple[torch.Tensor]] = None,
...
class Model(nn.Module):
    ...
    def __init__(self, config, load_head=False, load_emb=True, path=None):
        ...
        self.hidden_size = config.hidden_size
        self.draft_vocab_size = config.draft_vocab_size  # 压缩后的词汇表大小
        ...
        if not load_emb:
            ...
            import json
            import os
            safetensor_json = os.path.join(path, "model.safetensors.index.json")
            bin_json = os.path.join(path, "pytorch_model.bin.index.json")
            if os.path.isfile(safetensor_json):
                ...
                tensor = self.target_model.model.embed_tokens.weight
    ...
    def scandata(self, datapath, tokenizerpath):
        """
        扫描训练数据，统计token频率，构建压缩词汇表
        这是EAGLE的重要优化：只预测最常用的token
        """
